...
def load_span_sdd(model, i, j, k):
    prefix = "./sdd/span" + model + "/span" + model + "-" + str(k) + "-" + str(i) + "-" + str(j)
    root = load_t(prefix + ".txt")
    return root


def load_chained_sdd(n, k, formu):
    path = "../sdd/chained-pw" + formu + "/chained-pw" + formu + "-" + str(k)
    sdd_dict = dict()
    for i in range(2 ** n):
        prefix = ""
        for j in range(n):
            if (i >> j) % 2 == 0:
                ...
                prefix = "cl" + prefix
        sdd_dict[i] = load_t(path + prefix + "-.txt")
    return sdd_dict
