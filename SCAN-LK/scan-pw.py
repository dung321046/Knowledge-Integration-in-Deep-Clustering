...
from utils.train_lk import scan_train
...
def main():
    ...
    pw_test = 2
    from utils.constraint_utils import read_ml_cl
    pw_dict = dict()
    pw_dict["ml1"], pw_dict["ml2"], pw_dict["cl1"], pw_dict["cl2"] = read_ml_cl(
        r"SCAN-LK\constraints"'\\' +
        p['train_db_name'] + "-pw1000-0.5\\test" + str(pw_test).zfill(2))
    # Data
    print(colored('Get dataset and dataloaders', 'blue'))
    train_transformations = get_train_transformations(p)
    val_transformations = get_val_transformations(p)
    train_dataset = get_train_dataset(p, train_transformations,
                                      split='train', to_neighbors_dataset=True, pw_dict=pw_dict)
    ...
    if os.path.exists(p['scan_checkpoint']):
        ...
        checkpoint = torch.load(
            r"C:\Users\dung3\PycharmProjects\Unsupervised-Classification\results"'\\' + p[
