...
def get_train_dataset(p, transform, to_augmented_dataset=False,
                      to_neighbors_dataset=False, split=None, pw_dict=None, span=None):
    # Base dataset
    ...
    if span:
        from data.custom_dataset import NeighborsDatasetWithSpan
        dataset = NeighborsDatasetWithSpan(dataset, indices, p['num_neighbors'], pw_dict)
    elif to_neighbors_dataset:  # Dataset returns an image and one of its nearest neighbors.
        from data.custom_dataset import NeighborsDataset, NeighborsDatasetWithPW

        if pw_dict:
            dataset = NeighborsDatasetWithPW(dataset, indices, p['num_neighbors'], pw_dict)
        else:
...
def get_val_dataset_henry(dataset, topk_neighbors_path, to_neighbors_dataset=False):
    if to_neighbors_dataset:  # Dataset returns an image and one of its nearest neighbors.
        from data.custom_dataset import NeighborsDataset
        indices = np.load(topk_neighbors_path)
        dataset = NeighborsDataset(dataset, indices, 5)  # Only use 5
    return dataset
