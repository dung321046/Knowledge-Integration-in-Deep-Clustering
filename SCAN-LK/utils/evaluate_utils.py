...
@torch.no_grad()
def scan_evaluate(predictions, pw_dict=None):
    ...
    for head in predictions:
        ...
        stat_dict = {'entropy': entropy_loss, 'consistency': consistency_loss}
        pw_loss = 0
        if pw_dict:
            y_pred = head['predictions']
            nsat = 0
            for i, ml1 in enumerate(pw_dict["ml1"]):
                ml2 = pw_dict["ml2"][i]
                pw_loss += -0.1 * torch.log(torch.sum(probs[ml1] * probs[ml2]))
                if y_pred[ml1] == y_pred[ml2]:
                    nsat += 1
            for i, cl1 in enumerate(pw_dict["cl1"]):
                cl2 = pw_dict["cl2"][i]
                pw_loss += -0.1 * torch.log(1 - torch.sum(probs[cl1] * probs[cl2]))
                if y_pred[cl1] == y_pred[pw_dict["cl2"][i]]:
                    nsat += 1
            stat_dict['nsat'] = nsat
            stat_dict['pw_loss'] = pw_loss
            total_loss += pw_loss
        stat_dict['total_loss'] = total_loss
        output.append(stat_dict)
    ...
    final = {'scan': output, 'lowest_loss_head': lowest_loss_head, 'lowest_loss': lowest_loss}
    if pw_dict:
        nsats = [output_['nsat'] for output_ in output]
        highest_nsat_head = np.argmax(nsats)
        higest_nsat = np.max(nsats)
        final['highest_nsat_head'] = highest_nsat_head
        final['higest_nsat'] = higest_nsat
    return final
