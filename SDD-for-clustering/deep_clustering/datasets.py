...
class Reuters(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        if self.train:
            rtk10k_train = np.load(os.path.join(self.root, self.training_file), allow_pickle=True).item()
            self.train_data, self.train_labels = torch.tensor(rtk10k_train['data'], dtype=torch.float32), torch.tensor(
                rtk10k_train['label'], dtype=torch.int)
            if self.use_cuda:
                self.train_data = self.train_data.cuda()
                self.train_labels = self.train_labels.cuda()
        else:
            rtk10k_test = np.load(os.path.join(self.root, self.test_file), allow_pickle=True).item()
