...
def write_stat(filename, caption, data, arr_n, models, ref="tb-TBD", hidden_cols=[], draw=True):
    exported_table = []
    total_color_table = []
    idec_performance = {
    ...
    for i in range(len(data)):
        ...
        xc, nmic, nmic_err, accc, accc_err = [], [], [], [], []
        ...
        for n in arr_n:
            ...
            for fname, name in models.items():
                row = [data[i], n, name]
                full_name = "../test_set/Pw-" + data[i] + "/" + str(n) + "/" + fname + ".tsv"
        ...
        if draw:
            ...
            # ax.errorbar(xc, nmic, yerr=nmic_err, fmt='-.s', label="NMI-Cop-Kmeans", markersize=1, capsize=3)
            # ax.errorbar(xc, accc, yerr=accc_err, fmt='-.s', label="Acc-Cop-Kmeans", markersize=1, capsize=3)
...
if __name__ == '__main__':
    ...
    # n = [10, 100, 500, 1000]
    n = [10, 100, 500, 1000]
    models = {"cop-kmeans": "COP-Kmeans", "DCC200-pt": "DCC"}
    ...
    write_stat("./pairwise/pairwise", "Comparison on pairwise constraints with CopKmeans, DCC, and EDEC", data, n,
               models, ref="pw", hidden_cols=[6, 8])
    data = ["MNIST", "Fashion", "Reuters"]
    n = [1000]
    models = dict()
    for fomula in ["A5seed", "B5seed"]:
        # for batch in ["64", "128", "256"]:
        for batch in ["128"]:
            # for lambda_c in ["0.01", "0.001"]:
            for lambda_c in ["0.01"]:
                ...
                for t in ["sdd", "batch-sdd"]:
                    models[fomula + "-" + batch + "-" + t + "-" + lambda_c] = "EDEC-" + fomula[:1]
    write_stat("./pairwise/pw-stability", "Stability of pairwise EDEC for five different runs each test case", data, n,
               models, ref="pw-stability", hidden_cols=[6, 8], draw=False)
