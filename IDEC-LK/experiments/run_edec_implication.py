...
if __name__ == "__main__":
    ...
    v = 4
    m = 100
    folder_name = "../../SDD-for-clustering/generate_constraints/" + args.data + "-complexR3-" + str(
        v) + "-" + str(m)
    import timeit
    ...
    args.lr = 0.001
    lr2 = 0.001
    # setup_str = formu + "-" + str(lambda_c) + "-no-cluster-no-threshold-lr-0.1"
    setup_str = formu + "-" + str(lambda_c) + "-lr-" + str(args.lr)  # + "-no-pretrain"
    # setup_str = formu + "-" + str(lambda_c) + "-lr-" + str(args.lr) + "___" + str(lr2)
    ...
    for test in range(5):
        ...
        if args.data == "Reuters":
            ...
            dlcc = ComplexCS(input_dim=784, z_dim=10, n_clusters=10,
                             encodeLayer=[500, 500, 2000], decodeLayer=[2000, 500, 500], activation="relu", dropout=0,
                             lambda_c=lambda_c, formu=formu)
        if "no-pretrain" not in setup_str:
        ...
        dlcc.fit(stat, file_prob_name, arr_ids, complex_group, roots, v, X, y, case_folder + "/model.pt", lr=args.lr,
                 ...
                 tol=1e-4, lr2=lr2)
        # #     dlcc.fit(stat, file_prob_name, arr_ids, complex_group, mgrs, roots, X, y, lr=args.lr, num_epochs=args.epochs,
        # #              tol=-1)
